{{-- resources/views/class/edit.blade.php --}}
        ...
        <div class="bg-white shadow rounded-lg p-6">
            ...
            <form action="{{ route('class.update', $class->id) }}" method="POST">
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    {{-- Name --}}
                    <div>
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900">class Name</label>
                    ...
                    <div>
                        ...
                        <input type="text" id="price_display"
                            value="{{ old('price', $class->price) ? 'Rp. ' . number_format($class->price, 0, ',', '.') : '' }}"
                        ...
                        <input type="hidden" id="price" name="price" value="{{ old('price', $class->price) }}">
                        @error('price')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" rows="4"
                        class="bg-gray-50 border @error('description') border-red-500 @else border-gray-300 @enderror text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">{{ old('description', $class->description) }}</textarea>
                    @error('description')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit"
                        class="px-5 py-2.5 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm">Update</button>
                </div>
            </form>
        </div>
    </div>
